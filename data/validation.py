...
def validate_transaction_data(df: pd.DataFrame) -> bool:
    ...
    res_columns = ge_df.expect_table_columns_to_match_set(
        column_set=["amount", "type", "timestamp", "is_fraud"], exact_match=False
    )
    ...
    if not success:
        logger.error(
            "Data Quality Gate FAILED: Structural anomalies or unexpected distributions detected!"
        )
        return False

    logger.info(
        "Data Quality Gate PASSED: Ingestion schema satisfies all validation rules."
    )
    return True
