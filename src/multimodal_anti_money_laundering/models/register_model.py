...
def promote_best_model(
    experiment_name: str = "aml-baseline", model_name: str = "Multimodal_AML_Model"
):
    """Find the latest run from an experiment, register it, and promote it to Staging."""
    logger.info(
        f"Connecting to MLflow tracking server to scan experiment: '{experiment_name}'"
    )
    ...
    if "mlflow.log-model.history" in tags:
        ...
        try:
            ...
            if history:
                ...
                logger.info(
                    f"Dynamically detected logged model artifact path: '{artifact_path}'"
                )
        except Exception:
            pass
    ...
    try:
        ...
        logger.info(
            f"Model version V{model_version.version} successfully promoted to STAGING."
        )

    except mlflow.exceptions.MlflowException:
        logger.error(
            f"Registry Error: The run {latest_run_id} does not contain any logged model binaries."
        )
        logger.error(
            "Ensure that 'mlflow.xgboost.log_model()' or 'mlflow.pytorch.log_model()' is actively executing in your train script."
        )
