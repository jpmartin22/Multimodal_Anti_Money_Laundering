...
def run_text_training():
    ...
    with mlflow.start_run() as run:
        logger.info(
            f"MLflow active training session initiated. Run ID: {run.info.run_id}"
        )
        ...
        logger.info(
            f"Epoch Complete | Loss: {train_loss:.4f} | AUC-PR: {auc_pr_score:.4f}"
        )
        ...
        logger.info(
            "DistilBERT model binary successfully saved to MLflow artifacts tree."
        )
