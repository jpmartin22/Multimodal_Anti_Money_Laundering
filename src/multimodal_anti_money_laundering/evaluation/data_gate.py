...
def run_pre_training_validation(
    graph_features: np.ndarray, bilstm_sequences: np.ndarray, labels: np.ndarray
):
    ...
    try:
        ...
        label_success = res_label_values.get("success", False) or getattr(
            res_label_values, "success", False
        )
        null_success = res_label_nulls.get("success", False) or getattr(
            res_label_nulls, "success", False
        )
        ...
        if label_success and null_success:
            ...
            print(
                "❌ Labels Validation: FAILED (Invalid target characters or NaN values detected)"
            )
            all_passed = False
    ...
    if has_165_features and not has_graph_nans:
        print(
            f"✅ Graph Features Validation: PASSED (Shape matches (N, 165): {graph_shape})"
        )
    else:
        print(
            f"❌ Graph Features Validation: FAILED (Shape is {graph_shape} instead of (N, 165) or NaNs found)"
        )
        all_passed = False
    ...
    is_valid_bilstm_tensor = (
        len(bilstm_shape) == 3 and bilstm_shape[1] == 49 and bilstm_shape[2] == 165
    )
    has_bilstm_nans = np.isnan(bilstm_sequences).any()

    if is_valid_bilstm_tensor and not has_bilstm_nans:
        print(
            f"✅ BiLSTM Sequence Validation: PASSED (Shape matches (N, 49, 165): {bilstm_shape})"
        )
    else:
        print(
            f"❌ BiLSTM Sequence Validation: FAILED (Shape is {bilstm_shape} instead of (N, 49, 165) or NaNs found)"
        )
        all_passed = False
    ...
    if not all_passed:
        ...
        raise ValueError(
            "Data pipeline quality check failed. Halting training loop initialization."
        )
