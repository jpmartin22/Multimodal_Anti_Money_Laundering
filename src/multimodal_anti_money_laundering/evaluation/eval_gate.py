...
def check_metrics_gate(metrics_json_path: str = "reports/metrics.json") -> None:
    ...
    if not path.exists():
        logger.error(
            f"Evaluation Gate Failed: Metrics file '{metrics_json_path}' not found."
        )
        sys.exit(1)
    ...
    if auc_pr < TARGET_AUC_PR:
        logger.error(
            f"CRITICAL CRASH: Model performance ({auc_pr:.4f}) dropped below regulatory compliance target ({TARGET_AUC_PR})!"
        )
        sys.exit(1)

    logger.info(
        "SUCCESS: Model passed the evaluation gate. Proceeding to registration pipeline."
    )
    sys.exit(0)
